...
if [ "$PAPER" == "demo" ]; then
  ...
  # Swarm
  th main.lua -env rlenvs.Atari -modelBody models.Atari -game $GAME -cudnn true -height 84 -width 84 -colorSpace y -duel false -tau 10000 -PALpha 0 -eta 0.00025 -gradClip 0 -swarm true -action_coverage true "$@"
elif [ "$PAPER" == "agentcover" ]; then
  # Agent Coverage
  th main.lua -env rlenvs.Atari -modelBody models.Atari -game $GAME -cudnn true -height 84 -width 84 -colorSpace y -bootstraps 50 -duel false -tau 10000 -PALpha 0 -eta 0.00025 -gradClip 0 -swarm true -agent_coverage true "$@"
